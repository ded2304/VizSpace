# -*- coding: utf-8 -*-
__title__ = "Isovist Area Heatmap"
__author__ = "VizSpace"
...
GRID_SIZE    = 2.0  
TRANSPARENCY = 60
EPS          = 0.02   # tiny overlap to remove gaps between cells
...
op.print_md("# 🏢 Isovist Area Heatmap — VizSpace")
...
all_nodes = []   # (cx, cy, nz) — per-node elevation stored
...
# ── ISOVIST AREA METRIC ───────────────────────────────────────────────────────
op.print_md("## 📐 Computing Isovist Areas...")

cell_area = GRID_SIZE * GRID_SIZE
metric    = [len(adj[i]) * cell_area for i in range(K)]
...
op.print_md("Max isovist: **{:.1f} ft²** (Node {}) | Min: **{:.1f} ft²**".format(
...
<table style="border-collapse:collapse;font-family:sans-serif;font-size:13px;margin:8px 0;">
  <thead>
    <tr>
      ...
      <th style="padding:5px 10px;border:1px solid #ccc;">Isovist Area</th>
  ...
  <tbody>
    <tr>
      ...
      <td style="padding:4px 10px;border:1px solid #ccc;">Very Low</td>
      <td style="padding:4px 10px;border:1px solid #ccc;">Dead-ends, enclosed corners</td>
    </tr>
    <tr>
      <td style="background:#00FFFF;border:1px solid #ccc;"></td>
      <td style="padding:4px 10px;border:1px solid #ccc;">Low–Mid</td>
      <td style="padding:4px 10px;border:1px solid #ccc;">Private rooms, alcoves</td>
    </tr>
    <tr>
      <td style="background:#00FF00;border:1px solid #ccc;"></td>
      <td style="padding:4px 10px;border:1px solid #ccc;">Mid</td>
      <td style="padding:4px 10px;border:1px solid #ccc;">Secondary corridors, offices</td>
    </tr>
    <tr>
      <td style="background:#FFFF00;border:1px solid #ccc;"></td>
      <td style="padding:4px 10px;border:1px solid #ccc;">Mid–High</td>
      <td style="padding:4px 10px;border:1px solid #ccc;">Open areas, waiting zones</td>
    </tr>
    <tr>
      <td style="background:#FF0000;border:1px solid #ccc;"></td>
      <td style="padding:4px 10px;border:1px solid #ccc;">Very High</td>
      <td style="padding:4px 10px;border:1px solid #ccc;">Main halls, lobbies, junctions</td>
    </tr>
  </tbody>
</table>
...
t = Transaction(doc, "Isovist Area Heatmap")
t.Start()

new_view_id  = view.Duplicate(ViewDuplicateOption.Duplicate)
new_view     = doc.GetElement(new_view_id)
new_view.Name = unique_view_name(doc, view.Name + " - Isovist Area")
...
for idx, (cx, cy, nz) in enumerate(all_nodes):
    ...
    val = metric[idx]
    if val == 0.0:          # skip isolated nodes
        skipped += 1
        continue
    ...
    # ── OVERRIDE GRAPHICS (purple fix) ───────────────────────────────────────
    ogs = OverrideGraphicSettings()

    # both foreground AND background needed — missing background = purple
    ogs.SetSurfaceForegroundPatternId(solid_id)
    ogs.SetSurfaceForegroundPatternColor(color)
    ogs.SetSurfaceBackgroundPatternId(solid_id)
    ogs.SetSurfaceBackgroundPatternColor(color)

    # suppress black/purple projection line borders
